...
document.addEventListener('touchend', function (event) {
    ...
    //任务1：实现移动端触摸逻
    if(daltaX>Sen_vol && daltaX>Math.abs(daltaY))
    {moveRight();
        newBox(context);
        isGameOver();
    }

    if(daltaX<-Sen_vol && Math.abs(daltaX)>Math.abs(daltaY)){
        moveLeft()
        newBox(context);
        isGameOver();
    }

    if(daltaY>Sen_vol && Math.abs(daltaY)>Math.abs(daltaX)){
        moveDown();
        newBox(context);
        isGameOver();
    }
        ...
        for (var j = 1; j < 4; j++) {
            if (nums[i][j] != 0) {
                for (var k = 0; k < j; k++) {
                    ...
                    else if (nums[i][j] == nums[i][k] && noBlockVertical(i, k, j, nums)) {
                        nums[i][k] += nums[i][j];
                        nums[i][j] = 0;
...
function canMoveDown() {
    ...
    for(var i=0;i<4;i++){
        for(var j=2;j>=0;j--){
            if(nums[i][j]!==0){
                if(nums[i][j+1]==nums[i][j]||nums[i][j+1]==0){
                    return true;
                }
